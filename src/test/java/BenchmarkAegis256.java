...
public class BenchmarkAegis256 {
  ...
  public static void main(String[] args) {
    System.out.println("AEGIS-256 Benchmark Results");
    System.out.println("============================");
    System.out.println();
  ...
  private static BenchmarkResult benchmarkOperations(int msgSize, int adSize, int tagLength) {
    byte[] key = Aegis256.keygen();
    byte[] nonce = Aegis256.noncegen();
  ...
  private static void warmup(byte[] key, byte[] nonce, byte[] message, byte[] ad, int tagLength) {
    try {
      for (int i = 0; i < WARMUP_ITERATIONS; i++) {
        Aegis256 cipher = new Aegis256(key, nonce, tagLength);
        byte[] ciphertext = cipher.encrypt(message, ad);
        Aegis256 decipher = new Aegis256(key, nonce, tagLength);
